...
export const Transform = () => (
	<Paper sx={{ px:'2rem', backgroundColor: 'none', height: '100%' }} elevation={0}>
		<Grid container spacing={[0,2,4]} pt={[0,2]}>
			<Grid key={'title'} container item xs={12} direction="row" >
				<Typography variant={'h3'} py={2}>{transform.title} </Typography>
			</Grid>
			{ transform.content.map( (item, i) => {
				return(
					<Grid key={i} container item xs={12} sm={4} direction="column" >
						<Card sx={{ p:0, m:0, background: 'none', height: '100%' }} >
							<Typography variant={'h4'} pb={2}>{item.title} </Typography>
							<List>
